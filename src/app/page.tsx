...
export default async function ProtectedPage() {
    ...
    if (!session) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white">
                ...
                <Link href="/api/auth/signin" className="bg-blue-600 hover:bg-blue-700 px-6 py-3 rounded text-lg font-semibold">
                    Sign in with Google
                </Link>
            </div>
        );
    ...
    if (!role) {
        logger.warn("Unauthorized access attempt:", session.user?.email);
        redirect("/signin");
    }
