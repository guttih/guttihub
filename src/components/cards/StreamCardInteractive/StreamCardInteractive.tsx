    ...
    return (
        <div className="border rounded p-4 shadow bg-gray-900">
            <div className="flex justify-between items-center">
                ...
                <button onClick={() => setPlaying((p) => !p)} className="text-sm bg-blue-600 px-3 py-1 rounded">
            ...
            {playing && (
                <div className="mt-3">
                    <InlinePlayer serviceId={serviceId} movieTitle={entry.name} entry={entry} url={entry.url} />
                </div>
            )}
        </div>
    );
