...
== 🛠️ Setup Instructions for `guttihub`
...
[source,bash]
----
sudo dnf install -y nodejs npm postgresql postgresql-server postgresql-contrib ffmpeg jq curl at
...
CREATE DATABASE guttihubdb OWNER appuser;
ALTER ROLE appuser CREATEDB;
GRANT ALL PRIVILEGES ON DATABASE guttihubdb TO appuser;
\c guttihubdb
...
[source,bash]
----
touch .env && echo 'DATABASE_URL="postgresql://appuser:hunter2@localhost:5432/guttihubdb?schema=public"' >> .env
----

Test the connection (as postgres superuser):
[source,bash]
----
sudo -i -u postgres psql "postgresql://appuser:hunter2@localhost:5432/guttihubdb" -c "select current_user, current_database();"
----
...
# OAuth providers (enable the ones you use)
...
Available DB scripts in package.json:
- db:show: opens Prisma Studio
- db:generate: generates Prisma client
- db:reset: resets DB (skip seed) and regenerates client
---
