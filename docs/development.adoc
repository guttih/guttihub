...
This project uses the https://vitest.dev[Vitest] testing framework, which is fast, lightweight, and compatible with Jest-style syntax.

Tests live in the `test/` directory or close to source files (using `.test.ts` naming convention).
...
| Runs Vitest tests and generates a code coverage report.
|===

You can read more about Vitest at the official documentation: https://vitest.dev
