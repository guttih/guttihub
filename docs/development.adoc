...
[source,sh]
----
find . -type f ! -path "./node_modules/*" ! -path "./.next/*" ! -path "./.env.local" ! -path "./.vscode/*" ! -path "./.git/*" ! -path "./public/*" ! -path "./html/*" ! -path "./scripts/database/*" -print
----
...
.List all json files that are used to describe jobs or finished artifacts
[source,sh]
----
echo "🕒 $(date '+%Y-%m-%d %H:%M:%S')  🕒:Current time, 🍼:Yonger than 8 hours, 💀:Older than 8 hours  " && \
find . \
  \( -path "./node_modules" -o -path "./.next" -o -path "./.vscode" -o -path "./.git"  -o -path "./scripts" -o -path "./html" \) -prune -false \
  -o -type f -name "*.json" \
  ...
  ! -path "./src/config/services.json" \
  ! -path "./.cache/movie-consumers.json" \
  ! -path "./src/config/authorizedUsers.json" \
  ! -path "./src/config/output-dirs.json" \
  ! -path "./server/*" \
  -print0 | while IFS= read -r -d '' file; do
    mod_time=$(stat -c '%y' "$file" | cut -d'.' -f1)
    if [ "$(find "$file" -mmin +480)" ]; then
      echo "💀 $mod_time  $file"
    else
      echo "🍼 $mod_time  $file"
    fi
...
== Creating help from this document

If you have fetched the docker asciidoctor image, you can generate a help file from this document with:
[source,sh]
----
docker run --rm   -v "$(pwd)":/documents -w /documents   asciidoctor/docker-asciidoctor   asciidoctor docs/development.adoc
----
